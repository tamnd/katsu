...
The wall fell with `new`. 5,410 cases passed with 7 failures and 75,808 unsupported on the run before it, and 8,924 passed with 14,885 failures and 57,416 unsupported on the run after, which is the same suite finding out what is actually missing for the first time. The failure count going from 7 to 14,885 is the good news in that sentence and it is worth being explicit about why: a case that stops at `new Test262Error` is a case whose result nobody knows, and a case that fails is a case with a name, a message and a place to start. The work list at the bottom of the report went from one line about a constructor call to a ranked list of missing globals, which is the first time it has been a list of things to build rather than a single door.

The error constructors were the next 1,071. `assert.throws` takes the constructor it expects as its first argument, compares it against what it caught and reads `name` off it to report the difference, and `propertyHelper.js` asks `e instanceof TypeError` for every non writable and non configurable property it checks, so a suite that mentions an error constructor or `assert.throws` in 20,360 of the 53,874 JavaScript files under `test/` could not get a correct answer out of its own harness without them. The count went to 9,995 passing and 13,606 failing, and 208 cases moved from failing to unsupported rather than the other way, which is also the ratchet working: they now run far enough to reach a construct this build refuses by name instead of stopping at an undefined global.
