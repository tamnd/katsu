...
Deciding strictness is only half of it, because the mode also takes names away, and the adapter is where that is checked for the same reason it is where strictness is decided. There are two rules and they are not the same rule. Nine words, `implements` `interface` `let` `package` `private` `protected` `public` `static` and `yield`, stop being identifiers entirely, so `"use strict"; public;` is refused even though there is nothing to bind and nothing to write. `eval` and `arguments` stay perfectly good names to read and are only refused where a program moves one, which is a `var`, a `let`, a `const`, a function name, a parameter, a catch parameter or an assignment target. Reading `arguments` in strict code is a runtime `ReferenceError` if nothing declared it, not a syntax error, and getting that boundary wrong in either direction would refuse working programs or accept broken ones.

Two words are missing from that list of nine on purpose. `enum` is reserved in both modes rather than only in strict code, so the parser rejects it before the adapter sees it and with its own message. `await` is not a strict mode reserved word at all: what makes it special is a module or an async function, and neither of those is strictness. A property name is never checked by either rule, which is why `o.public` and `{ public: 1 }` are both fine in strict code, and the two never meet because a property name is adapted at its own site rather than through the identifier path. The function name is the one case that has to wait, since it is checked against the function's own strictness rather than the enclosing strictness, so `function eval() { "use strict"; }` is refused from a sloppy file. All nine words and both rules were run through Node in every position before any of this was written, and the two messages are Node's exact words because they end up in somebody's test expectations.

The M0 subset is literals, identifiers, `this`, the unary, binary, logical, update, assignment and conditional operators, member access, calls, functions, `var`, `let`, `const`, `if`, `while`, blocks, `return` and expression statements. M1 adds `switch`, `break`, `continue`, labels, `do while`, the C style `for`, object literals, `throw` and `try` with a `catch` or a `finally` or both. Everything else is refused with the construct named the way a JavaScript programmer would name it, and a line and a column. The refusal list is the M1 and M2 work list read backwards and it should shrink to nothing. The alternative, quietly producing bytecode for syntax the frontend does not understand, is worse than admitting the gap.
...
The three loop forms are three layouts rather than one layout with a flag, because the whole difference between them is where the test sits relative to the body. A `while` puts the test at the top and the back edge at the bottom. A `do while` puts the body at the top, and its `continue` goes to the start of the test rather than to the back edge, because landing on the back edge would skip the condition and make `do { continue; } while (false)` run forever. A `for` is head, test, body, update, back edge, so its `continue` lands on the update and its `break` lands past the back edge, and that is what makes a `continue` in a counting loop safe when the same `continue` in the `while` it desugars to would not be. An absent `for` test emits no comparison at all rather than a comparison against a constant, so `for (;;)` is a byte tighter than `while (true)` and needs no implicit return after it, since nothing can reach past the back edge.

A label is a name for a statement and it is not a binding, so labels and variables are two namespaces that never meet and `let x = 1; x: while (0);` is a legal program. What a label does share with a variable is the spelling, since it is written with the identifier production, so `public:` is refused in strict code the same way reading `public` is. Labels do not cross a function boundary in either direction, which is what stops a labelled jump from becoming a strange kind of return, and a duplicate is an early error only when one label encloses the other, so two labelled statements side by side may share a name. A `break` may name any labelled statement at all, including a block, an `if` and a `try`, and a `continue` may only name one that denotes an iteration statement, looking through a chain of labels to whatever the innermost one is on. All three messages are Node's word for word.

Lowering a labelled jump is a change to the frame stack rather than a new statement shape. Each loop and each switch already pushes a frame that collects the jumps aimed at it, so a labelled statement that is not one of those pushes a frame that collects breaks and nothing else, and a label written on a loop or a switch is handed to that loop's own frame instead of building a second one around it. A jump then searches outwards for the frame wearing the name rather than taking the nearest, and a labelled statement with no jump aimed at it emits no instruction at all.

The case that needs more than a search is a labelled jump that leaves through a `finally`. A completion travelling through a `finally` is a token in a register that the dispatch after the body reads, and by the time that dispatch runs the frame the jump was aimed at is gone, so the token has to say which target it meant. Each distinct labelled target that routes through one `finally` gets a token value of its own, allocated per function and deduplicated, so a `finally` with no labelled jump through it compares exactly the numbers it always did and pays nothing for the feature.

The head of a `for` opens one block scope that covers the init, the test, the update and the body. A `var` head hoists to the function and outlives the loop, a `let` or `const` head is invisible outside it, and a `let` in the body shadows a head of the same name rather than redeclaring it. The head is declared before its own initialiser is walked, which is the only reason `for (let i = i; ;)` is a dead zone error rather than a read of an outer `i`. A `const` head with no initialiser never reaches scope analysis, because the parser refuses it first and does it in Node's exact words.

One thing about loops is knowingly wrong today and it is recorded here rather than left to be discovered. A `let` head gets one binding per call and not one per iteration, because environments in this engine are per function and the cells are allocated once when the frame is built. So `for (let i = 0; i < 2; i++)` with a closure made in each iteration gives `0 1` in Node and `2 2` here. This is not something the loop lowering introduced, since the same divergence reproduces on a `while` around a block, and it is not something the loop lowering can fix on its own. It needs block level environments, which is the environment representation work in M1, and it is tracked there.
...
`throw` is one opcode and it is a terminator, so nothing is emitted after one and a function whose last statement is a `throw` gets no trailing return.

`finally` is not a third clause beside the other two, because a `catch` runs on one way out of a block and a `finally` runs on all five. It cannot be another entry in the table, since the table only knows about throwing. What it is instead is a body with one entry point, reached from the normal path and from every abrupt one, and a dispatch after it that sends the completion on to wherever it was going. Two registers carry that across the body: a token saying which of the five ways out this was, and a payload holding what the completion is carrying, which is a thrown value or a returned one. A normal completion sets the token to zero, and zero is falsy, so the dispatch is one `jump_if_false` on the path nearly every `finally` takes. A throw needs no jump into the body at all, because the handler entry can name the body's prologue directly and the search has already stored the value in the payload register.

The dispatch only asks about the completions that actually routed through, which is what keeps the usual shape down to two instructions. A `try` and `finally` with nothing abrupt inside it has one `jump_if_false` for the normal case and one `throw` for the only other thing the token could have been, with no comparison anywhere. Each further kind adds one comparison and one jump, and the last kind never needs a test, because a token that is not zero and is not any of the kinds already asked about can only be the one remaining.

The frame is pushed while the protected block is lowered and popped before the body is lowered, and that ordering is the whole of the override rule. A `return` written inside a `finally` body is lowered against whatever is outside the construct, so it leaves rather than routing back into the body it is written in, and the completion that was pending is dropped because the dispatch is never reached. That is why `try { return 1; } finally { return 2; }` answers two. Nesting needs nothing that knows about nesting either: each dispatch arm re issues its completion through the same three functions an ordinary statement goes through, so an inner `finally` hands a `return` to the outer one by itself, and a throw needs no chaining because the `throw` it emits already sits inside whatever range guards it.

`try B catch C finally F` is lowered as `try { try B catch C } finally F`, which is how the standard defines it and gets the table order right for free: the inner entry is pushed when the catch's protected block finishes and the outer one when the whole `try` and `catch` does, so a throw in `B` finds `C` and a throw in `C` finds `F`.
