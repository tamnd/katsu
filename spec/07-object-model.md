...
### 7.4.1 The heap kinds and the word that tells them apart, as built

The cage holds nine kinds of object. `crates/katsu-gc/src/function.rs` adds a closure, a context, a function written in Rust and the accessor pair a getter and setter share a slot in, `crates/katsu-gc/src/shape.rs` and `crates/katsu-gc/src/ordinary.rs` add a shape, an ordinary object and the properties array an object overflows into, `crates/katsu-gc/src/elements.rs` adds the indexed properties, and the string was there before any of them. The question every pointer has to answer is which of them it points at. The word they share and the reads and writes that reach it live in `crates/katsu-gc/src/object.rs`, which is where they went once they stopped being about functions.
...
A function is an object, and the last field is how. Everything a program hangs off a function is an ordinary property: `Foo.prototype`, the statics on a constructor, `Object.create` on a builtin. The obvious way to give a function properties is to put a shape in its first word and let it be an ordinary object, which is what a mature engine does and is not what this is, because the first word is what tells the kinds apart and reading a shape to re-derive what a tag already says would put a dereference on the hottest path in the runtime. So the properties live in an ordinary object of their own that the function points at, whose prototype is `Function.prototype`. Prototype chains and inline caches then work on it unchanged, and a read of `Foo.prototype` caches exactly as a read of `o.x` does. The field is zero until something asks, so a function that is only ever called pays four bytes and no allocation, and the function that is read from builds the object and its `prototype` together, since a function somebody reads a property off is usually a constructor.

A function written in Rust is twelve bytes: the kind tag, an ordinal into a table the isolate owns, and the same properties field for the same reason, since `Object` is a function with `create` and `prototype` on it. What it deliberately does not hold is the code. A function pointer is eight bytes in a four byte world, and it points into the text segment of the process rather than into the cage, so putting one in a heap object would be a pointer the collector has to know not to trace and the compression story has to make an exception for. An ordinal is a small integer that means nothing outside the isolate that issued it, which is exactly the property every other reference in the cage already has. The name lives next to the code in that table for the same reason, since a builtin's name is fixed at the point the realm is built and never comes from the heap.
...
An ordinary object is a sixteen byte header of a shape, a properties array, an inline capacity and an elements array, followed by its inline slots. A value is eight bytes here for exactly the reason a context cell is eight bytes, written down above, and it goes back to four when there is a heap number and a set of realm singletons for a slot to point at.

The elements word 7.4 puts in the header is the fourth word rather than the second, and it is the padding word that used to sit between the inline capacity and the first slot. The header was already sixteen bytes, because a value has to start on an eight byte boundary and three four byte fields do not reach one, so the word was there and unread whether anything used it or not. Putting elements in it moves no offset, grows no object and costs no store: an object with no indexed properties has a zero there, which is what the heap handed out.
...
A literal is built by the same stores as everything else, which is the part of this that pays for itself. `{a: 1, b: 2}` lowers to an empty object with room for two and then two ordinary property stores, so it walks the same transition path an object built by two assignments walks and arrives at the same shape node. There is no second code path for literals to disagree with the first one on, the room is a promise about what is coming rather than a limit, and a duplicate name in a literal comes out right without anything special being written for it, because storing twice and keeping the position of the first store is what a store already does.
...
### 7.6.1 A computed key is a property name, unless it is an index

`o[k]` is `ToPropertyKey` and two opcodes: the key is converted to text, the text is interned, and the interned name goes into the same shape search a dotted access uses. Every key in the language is a string or a symbol and there are no symbols yet, so that is the whole of the conversion, and it is why `o[0]` and `o["0"]` are one property rather than two.

The cost of doing it that way was measured rather than asserted, on the same machine in one session, before there was anywhere else to put an index. A named read is about 10 nanoseconds. The same read through a key already held as a string is about 20, which is the intern table lookup plus the absence of a cache. The same read through a number was about 156, which is fifteen times the named read, because every access formatted the number into text, allocated the text, hashed it and interned it before any of the search started.

That third number was the argument for element storage rather than a reason to be unhappy about the second. `a[i]` in a loop is the shape most real code touches an array with, and the conversion is pure waste in every one of those accesses because the integer was already the answer. 7.6.3 is what happened to it. What is left in this subsection is the path a key that is not an index still takes, which is every string key and every object with a `toString` on it, and that path is unchanged.
...
### 7.6.2 The element storage, as built

`crates/katsu-gc/src/elements.rs` is the storage the previous subsection says is missing, and the elements word 7.4.2 describes is what points at it. It is one array per object, addressed by the integer: a kind tag, a capacity, and then eight byte values. No names in it, because the index is the name, which makes it the same shape as the properties overflow array next door and for the same reason.

Indices are not in the shape and that is the whole point. A shape is shared by every object with the same properties in the same order, and no two arrays keep the same set of indices for long, so an index in a shape means a transition tree that grows a node per element and is shared with nothing. It also means turning the integer into text and interning it on every access to find a name that was an integer to begin with, which is the 156 nanoseconds measured above.

A hole is `Value::EMPTY`, whose bit pattern is zero, and it costs nothing to write because the bump heap hands out committed pages that are already zero. A fresh array is entirely holes without one store, and growing one by copying into a larger allocation leaves the new tail as holes for free. `[1, , 3]` costs what its two values cost.

There is one other word in the array that is not a value, and it is one. It means the property at that index is on the object under the text of the number instead, so the reader has to go and look for it there and the writer has to leave it there. Two things put it in and 7.6.3 has both. Zero and one being the two smallest numbers is what makes the test on the read path a single comparison, and neither of them is a bit pattern any value encoding produces.

There is no length in the array, on purpose. An array's `length` is an own property of the array, it is writable, and `a.length = 100` has to work without allocating a hundred slots. An ordinary object with `o[5] = 1` on it has no length at all, and enumeration wants the indices that are really there, which means skipping holes over the capacity whether a length is recorded or not. A third number that agrees with the other two most of the time is a bug waiting for the case where it does not.

The storage is measured against the named path it replaces, in `crates/katsu-gc/benches/objects.rs`, on the m4 reference machine from 15.5 and in one session. Reading an index out of an object with sixteen elements on it is 0.73 nanoseconds. Reading the cheapest named property off an object with sixteen of those, the one added last and therefore at the near end of the parent chain, is 1.35. Reading the dearest, the one added first, is 16.1. So the element read is roughly twice the best named read and twenty two times the worst, and unlike the named read it is the same number at every size, because it walks nothing. None of these three include the number formatting a computed key pays in the interpreter today, which is the larger half of the 156 nanoseconds above and the part that goes away entirely.

Building sixteen elements one at a time costs 53 nanoseconds including the object, and asking for the room up front costs 41. That gap is the four allocations the doubling makes against the one a literal needs, and it is why `reserve_elements` exists as a separate entry point rather than letting every array literal grow into place.

Writing past the end grows the array by doubling, so a loop appending one element at a time allocates a logarithmic number of times. Writing far past the end does not: past a gap of 1024 the write is refused and the caller is expected to store the value under the text of the number instead, because the array that would hold `o[100000] = 1` is eight hundred kilobytes of holes around one value. The number is a policy choice rather than a measurement and it is written down as one.

What this subsection does not yet include is the specialized matrix at the top of 7.6. Every element is eight bytes and every array is holey, which is the same deviation from the memory story that context cells and property slots make, for the same reason: a slot has two things it can be and a double is neither of them until there are heap numbers. The kinds arrive when the storage has a real array object using it and something to measure them against.

### 7.6.3 The index fast path, as built

The storage in 7.6.2 is reached by the two index opcodes before the key is ever spelled. `GetIndex` asks whether the key is an array index, and if it is and the receiver is an object, it loads the element and is done. `SetIndex` does the same on the way in. An index is an integer from zero to `2^32 - 2`, and the top value is excluded because the language excludes it: `a[4294967295]` is a property name and not an index, which is what lets an array's `length` be `2^32 - 1` and still be one past the last index. That was checked against Node rather than remembered.

A key arrives in three shapes and all three are accepted. An integer is the loop counter and is the reason any of this exists. A double is `a[i / 2]` landing on something whole, and negative zero counts as index zero because `ToString` spells `-0` as `"0"`. A string is `o["0"]` and the object literal key `{'0': 1}`, and it is accepted only in the spelling `ToString` would produce, so `"0"` is index zero while `"00"`, `"0.0"`, `" 0"` and `"+0"` are ordinary names. Node agrees: `{'0': 'b', '00': 'a'}` prints as two properties.

The rule that keeps this correct is that an index is in exactly one of the two places and never in both, because a property that lives in two places answers with whichever one the reader looked at first. `DefineValue` routes an index spelled key to the element storage, so `{'0': 1}` and `o[0] = 1` produce the same object. `Object.defineProperty` with an index and the default flags does the same.

Two things put an index under a name anyway. One is the sparse refusal from 7.6.2, where the array that would hold the index is mostly holes. The other is `Object.defineProperty` with anything other than the default flags, because the element storage holds values and it does not hold flags, so a hidden or a read only index has nowhere else to go. Both are rare and both would otherwise be the two places problem, so both are closed, in three parts.

A store that would make a new element, meaning one at or past the capacity, first asks whether the object already holds that index as a name and leaves the value there if it does. That costs a format and an intern and it is only ever on the path that is already allocating and copying, so the loop that appends never reaches it. When the array does grow, every own name that spells an index the array has just grown out over is marked in the new array with the one word from 7.6.2, so the fast path finds the mark and goes to the name. That sweep walks the shape chain, which is the smaller half of an allocate and copy, and it happens a logarithmic number of times in the length rather than once per write. Nothing is interned in it: the question is whether a name that already exists spells an index, which is a scan of at most ten characters. And a define that has to use a name marks the slot itself, which covers the case where the array already reaches that far.

The write fast path pays one comparison for all of this, against a word it is about to overwrite anyway, so the read that finds the mark is on a cache line the store has already brought in.

Reads fall through rather than stopping. An element that is a hole, or that is the marked word, means the index may still be a name, on this object or above it, so the search continues. That search walks the chain once and asks each level for its element before its name, which is the only order that gets both halves of shadowing right: an element on an object has to hide a name of the same text on a prototype, and a name on an object has to hide an element on a prototype. Two separate walks get one of those wrong whichever way round they run.

Enumeration puts the indices first and in ascending order whatever order they arrived in, which is what the language specifies and what Node was measured doing: `b.x = 1; b[0] = 2` prints `{ '0': 2, x: 1 }`. The element storage gives that for free because the array is already in index order and the shape is already in insertion order. An index that had to go under a name is pulled back out of the names and sorted into place, which is a sort of an almost sorted list and usually compares nothing at all.

The numbers, from `crates/katsu-vm/benches/interpret.rs` on the m4 reference machine, measured back to back against the same benchmark with the fast path taken out, because the machine drifts enough between sessions that only a pair run one after the other says anything. A thousand reads through a number went from 292 microseconds to 4.8, so 292 nanoseconds per read down to 4.8, a factor of 61. A thousand writes went from 221 microseconds to 9.9, a factor of 22. The reason both are larger than the 156 nanoseconds in 7.6.1 predicted is that the machine was warmer for this pair than for that one, which is exactly why the ratio is the claim and the absolute numbers are not.

The shape of the result matters more than the size of it. A read through a number is now the fastest property read in the benchmark, at about 5 nanoseconds against 13 for a dotted name and 20 for a key held as a string, because it touches neither the intern table nor a shape. That is the right ordering: an index should be the cheapest thing in the object model, and until this it was the dearest by a factor of fifteen.
