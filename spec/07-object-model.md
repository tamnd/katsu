...
### 7.4.1 Four heap kinds before there are shapes, as built

Shapes are M1 work and calls are M0 work, so the cage holds four kinds of object today with nothing to describe them. `crates/katsu-gc/src/function.rs` adds three of them, a closure, a context and a function written in Rust, to the string that was there already, and the question every pointer now has to answer is which of the four it points at.
