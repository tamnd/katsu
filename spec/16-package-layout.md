...
The crates go up from the release workflow rather than from anybody's laptop, through `cargo xtask publish`, which takes the order from the dependency graph and uploads one crate at a time. A publish cannot be taken back, so everything about it that can be checked without a tag is checked on every commit instead: `cargo xtask release` reads the workspace and fails on a version that is not in lockstep, on an internal dependency asking for a version other than the one being released, and on a missing description or license, and CI packages every crate the way crates.io would with `cargo publish --workspace --dry-run`, which verifies the whole set builds as if it were already published. The tag goes through the same check on the way in, so a tag that does not match the version in the tree stops before the upload rather than publishing the previous release under the new name.

Uploading one crate at a time rather than letting `cargo publish --workspace` do it is a decision the registry forced. crates.io lets one account create a burst of five crate names and then one every ten minutes, and there is no flag anywhere in cargo for skipping a version that is already uploaded, so a first release of fifteen crates stops partway through with a 429 and no way to resume. That is what happened to 0.1.7, which got one crate up before it stopped. Asking the registry per crate whether the version is there makes a stopped release finish on a re-run, and reading the time out of the rate limit message makes it wait rather than fail. The cost is that a first release of a new crate name is slow and every release after it is not, because the limit for new versions of an existing crate is a burst of thirty.

Waiting only when the registry says to is not enough, which 0.1.8 found out over three runs that got no further than 0.1.7 did. The bucket charges a token for a request it refuses as well as for one it accepts, so an upload that asks the instant the message says it may is refused and takes the refilled slot away from the attempt after it, and the deadline moves another ten minutes out every time. A publish therefore paces itself between new crate names rather than reacting, and when it is refused twice it waits a whole refill period past the second deadline, because two periods of not asking is what it takes to get a token back. The publish is also a single lane: the release workflow and the manual one share a concurrency group, since two runs uploading at once would spend the same allowance on each other's refusals.

The manual one is `Publish crates`, which takes a tag name and runs the same publish from the default branch. It exists because re-running the release job on a tag runs the code as it was at that tag, so a release stopped by a bug in the publish cannot be finished by the thing that has the fix. It uploads the same version either way, because the release check refuses a tree whose version is not the tag it was given.

The token that authorizes the upload is a secret on a GitHub environment that takes deployments only from a tag matching `v*`, so nothing running on a branch or on a pull request can reach it. It is a long lived token only because crates.io cannot attach a trusted publisher to a crate that has never been published, and it is replaced by trusted publishing once the first release exists.
