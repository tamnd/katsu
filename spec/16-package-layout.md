...
The crates go up from the release workflow rather than from anybody's laptop, in one `cargo publish --workspace` that takes the order from the dependency graph and verifies the whole set builds as if it were already published before it uploads any of it. A publish cannot be taken back, so everything about it that can be checked without a tag is checked on every commit instead: `cargo xtask release` reads the workspace and fails on a version that is not in lockstep, on an internal dependency asking for a version other than the one being released, and on a missing description or license, and CI packages every crate the way crates.io would. The tag goes through the same check on the way in, so a tag that does not match the version in the tree stops before the upload rather than publishing the previous release under the new name.
