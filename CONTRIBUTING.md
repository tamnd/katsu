...
Expect a first release of a new crate name to be slow. crates.io lets an account create a burst of five crate names and then one every ten minutes, which is what stopped 0.1.7 after one crate, so fifteen new names is a little over two hours of a job sitting still. The publish keeps to that pace on its own, and a re-run picks up where a stopped one left off. Once the names exist the limit is the one for new versions of existing crates, which is a burst of thirty, and the whole upload takes minutes.

Do not retry a rate limited publish by hand and do not run two of them at once. The bucket charges a token for a request it refuses as well as for one it accepts, so asking again the moment the message says you may takes the slot away from the attempt that would have succeeded, and a run that keeps doing it never publishes anything again. That is what held 0.1.8 at one crate through three runs, and it is why the two workflows that can publish share a concurrency group.

If a release stops in a way that needs a change to the publish itself, the fix goes on main and `Publish crates` in the Actions tab finishes the release from there, with the tag name as its input. Re-running the job on the tag would run the code as it was at the tag, which is the code that stopped. Nothing about the version changes: the same check refuses to upload a tree whose version is not the tag being finished.
