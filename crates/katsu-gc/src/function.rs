...
fn slot_of(cage: &Cage, pointer: NonNull<u8>) -> Option<Slot> {
...
unsafe fn write_u32(pointer: NonNull<u8>, at: usize, value: u32) {
    // SAFETY: the caller guarantees the write is inside the allocation. Every offset this module
    // passes is a header word, and a header word is four byte aligned inside an object the heap
    // aligned to eight, which is what clippy cannot see through a byte pointer.
...
unsafe fn read_u32(cage: &Cage, offset: u32, at: usize) -> u32 {
...
unsafe fn write_u64(base: *mut u8, at: usize, value: u64) {
    // SAFETY: the caller guarantees the write is inside the allocation. The only eight byte fields
    // here are context cells, which start at a sixteen byte header and are eight bytes each, so
    // every one of them is eight byte aligned inside an eight byte aligned object.
    #[allow(clippy::cast_ptr_alignment)]
    unsafe {
...
/// # Safety
///
/// `base` must be the start of an allocation at least `at + 8` bytes long, with that cell written.
unsafe fn read_u64(base: *mut u8, at: usize) -> u64 {
    // SAFETY: as `write_u64`, and every cell is written when the context is created.
    #[allow(clippy::cast_ptr_alignment)]
    unsafe {
